...
Entities in `PREDICATION` are identified by a *concept unique identifier* (CUI) in the column `SUBJECT_CUI` or `OBJECT_CUI`. The most CUIs are atomic strings (`C0039258`) while 
...
```
[semmed@nlm.nih.gov](mailto:semmed@nlm.nih.gov) has been requested to document the (implicit) relationship of individual ID parts.
...
Once the MySQL databse is ready configure the Stardog server. It is recommended to increase the [available process memory](https://www.stardog.com/docs/#_memory_usage), e.g. `export STARDOG_SERVER_JAVA_ARGS="-Xms30g -Xmx30g -XX:MaxDirectMemorySize=30g"` and specify the [memory mode](https://www.stardog.com/docs/#_memory_configuration) in `$STARDOG_HOME/stardog.properties`: `memory.mode=write_optimized`. Restart the server to apply the changes and revise the effective settings via `stardog-admin server status`.
...
![Sample constraints](figures/Constraints.png "Sample constraints")


The ICV rule [`contradicting_predications_count.ttl`](constraints/contradicting_predications_count.ttl) applied to the `semmeddb`database revealed 112 negating predications to conflict:
